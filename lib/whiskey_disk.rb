...
class WhiskeyDisk
  class << self
    ...
    def encode_roles(roles)
      note "Provides roles #{roles.inspect}"
      return '' unless roles and !roles.empty?
      "export WD_ROLES='#{roles.join(':')}'; "
    end

    def build_command(domain, cmd)
      "#{'set -x; ' if Config.debug_shell?}" + encode_roles(domain[:roles]) + cmd
    end
    ...
    def ssh(domain, cmd)
      note "Running command on [#{domain}]: [#{cmd}]" if Config.debug_any?
      args = [domain[:name], build_command(domain, cmd)]
      args.unshift '-v' if Config.debug_ssh?
      system('ssh', *args)
    end

    def shell(domain, cmd)
      note "Running command locally: [#{cmd}]" if Config.debug_any?
      system('bash', '-c', build_command(domain, cmd))
    end

    def echo(msg=nil)
      "echo '[PLACEHOLDER] #{msg}'"
    end

    def note(msg=nil)
      puts "[#{self.active_domain}] #{msg}"
    end

    def flush
      needs(:domain)
      self[:domain].each do |domain|
        self.active_domain = domain[:name].split("@").last
        next unless domain_of_interest?(domain[:name])
        note "Deploying #{domain[:name]}..."
        to_run = bundle.gsub("PLACEHOLDER", self.active_domain)
        status = remote?(domain[:name]) ? run(domain, to_run) : shell(domain, to_run)
    ...
    def if_task_defined(task, cmd)
      puts "Task: #{task}, cmd: #{cmd}"
      %Q(rakep=`#{env_vars} bundle exec rake -P` && if [[ `echo "${rakep}" | grep #{task}` != "" ]]; then #{cmd}; fi )
    end
    ...
    def refresh_checkout(path, repo_branch)
      enqueue "cd #{path}"
      enqueue "git fetch origin +refs/heads/#{repo_branch}:refs/remotes/origin/#{repo_branch} #{'&>/dev/null' unless Config.debug_shell?}"
      enqueue "git checkout #{repo_branch} #{'&>/dev/null' unless Config.debug_shell?}"
      enqueue "git reset --hard origin/#{repo_branch} #{'&>/dev/null' unless Config.debug_shell?}"
    end
    ...
    def run_rake_task(path, task_name)
      ...
      enqueue(if_file_present("#{self[:deploy_to]}/Rakefile",
        if_task_defined(task_name, "#{env_vars} bundle exec rake #{'--trace' if Config.debug_shell?} #{task_name} to=#{self[:environment]}")))
    end
    ...
    def run_script(script)
      ...
      enqueue(%Q<cd #{self[:deploy_to]}; #{echo "Running post script..."}; #{env_vars} bash #{'-x' if Config.debug_shell?} #{build_path(script)}>)
    end
    ...
    def refresh_configuration
      ...
      enqueue("rsync -a#{'v --progress' if Config.debug_shell?} " + '--log-format="%t [%p] %i %n" ' +
